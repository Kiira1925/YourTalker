...
export function compileCharacterInstructions(character: CharacterProfile): string {
  const profile = [
    ...
    profileLine('年齢・年代', character.age),
    profileLine('性別・ジェンダー', character.gender),
    profileLine('種族・存在区分', character.species),
    profileLine('職業・役割', character.occupation),
    profileLine('外見・服装', character.appearance),
    ...
    profileLine('目的・望み', character.goals),
    profileLine('能力・得意分野', character.abilities),
    profileLine('弱点・不得意', character.weaknesses),
    profileLine('恐れ・コンプレックス', character.fears),
    profileLine('世界観・時代・場所', character.world),
    profileLine('生い立ち・経歴', character.history),
    profileLine('所属・立場', character.affiliations),
    profileLine('秘密・隠し事', character.secrets),
    profileLine('ユーザーとの関係', character.relationship),
    profileLine('行動傾向', character.behaviorStyle),
    profileLine('癖・習慣', character.habits),
    profileLine('感情表現', character.emotionalExpression),
    profileLine('一人称', character.firstPerson),
    profileLine('他者の呼び方', character.addressingOthers),
